...
no server, no account and no telemetry. The only thing it keeps between runs is `settings.json`.
...
`GitHubReleaseSource`, and `AppDataPaths` for where files go.
...
bumps `AppSettings.CurrentVersion` and migrates in `Normalized`. There is no database and no other
stored state.
