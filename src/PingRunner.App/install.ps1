...
function Get-AppVersion {
    ...
    {
        if (-not [string]::IsNullOrWhiteSpace($propertyGroup.PingRunnerVersion))
        {
            return $propertyGroup.PingRunnerVersion.Trim()
        }
    }

    throw "Could not find a <PingRunnerVersion> element in $ProjectPath."
}
...
$projectPath = Join-Path -Path $PSScriptRoot -ChildPath 'PingRunner.App.csproj'
$appVersion = Get-AppVersion -ProjectPath (Join-Path -Path $PSScriptRoot -ChildPath '..\..\Version.props')
